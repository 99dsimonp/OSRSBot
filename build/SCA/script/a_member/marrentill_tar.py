...
from data.script.a_member import marrentill_tar_config as config

class marrentill_tar(threading.Thread):
        ...
        while self.STATUS_0:
            if self.SCRIPT_PHASE == 0:
                ...
                if self.engine.getObject(self.engine,self.config.CHATBOX[0]):
                    self.xy = self.engine.getSize(self.engine,self.config.CHATBOX[0])
                    self.input.moveMouse(self.input,self.xy[0][0],self.xy[1][0],0.1,1,self.config.CHATBOX[0],1)
                    self.SCRIPT_PHASE = 1
                else:
                    self.SCRIPT_PHASE = 1
                ...
                if self.engine.getObject(self.engine,self.config.FUNCTION[0]):
                    ...
                    self.input.moveMouse(self.input,self.xy[0][0],self.xy[1][0],0.1,1,self.config.FUNCTION[0],45)
                    self.SCRIPT_PHASE = 0
                ...
                self.sca.logoutInit(self.sca,1)
                exit()
            else:
                pass
